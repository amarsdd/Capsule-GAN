    ...
    for i in range(r):
        for j in range(c):
            if datasets == 'mnist':
                ...
                axs[i,j].axis('off')
                cnt += 1
    fig.savefig("images_Gcaps_%s/%d.png" % (datasets,epoch))
    plt.close()
...
def save_model():
    ...
    save(generator, datasets+"_capsGgan_generator")
    save(discriminator, datasets+"_capsGgan_discriminator")
    save(combined, datasets+"_capsGgan_adversarial")
...
np.savez(datasets+'capsGgan.npz',Class_Acc=Class_Acc, D_L=D_L, G_L=G_L, D_L_REAL=D_L_REAL, D_L_FAKE=D_L_FAKE)
...
plt.savefig(datasets+'capsGgan_all_loss.png')
plt.show()
...
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.savefig(datasets+'capsGgan_Classification_Accuracy.png')
plt.show()
...
#     sampled_labels = to_categorical(sampled_labels, num_classes=num_classes)
    ...
    plt.savefig(datasets+'capsGgan_all_distributions.png')
    plt.show()
